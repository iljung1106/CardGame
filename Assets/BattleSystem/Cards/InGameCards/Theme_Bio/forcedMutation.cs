...
public class forcedMutation : Card
{
    ...
    override public void SetCard(Card selectedCard)
    {
        if (isPlayers && selectedCard.IsMonster())
        {
            ...
            isUsed = true;
            selectedCard.GiveDamage(4, this);
            if(selectedCard.GetHealth() <= 0)
            {
                controller.SetCardOnTomb(this);
                return;
            }
            int unitCost = selectedCard.GetCost() + 3;
            ...
            {
                selectedCard.SetCost(selectedCard.GetCost() + 3);
                selectedCard.SetAttackDamage(selectedCard.GetAttackDamage() + 3);
                selectedCard.SetHealth(selectedCard.GetHealth() + 3);
            }
